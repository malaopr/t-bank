...
```bash
du -h logs/*.logs | sort -rh | head -n 1 | awk '{print $2}' > biglog.txt
```
...
```bash
grep -r -i "host" logs/config/ > host_params.txt
```
...
```bash
powershell.exe -Command "Compress-Archive -Path 'logs/config' -DestinationPath 'config_backup.zip'"
ls *.zip
```
...
```bash
powershell.exe -Command "Compress-Archive -Path 'logs/config/*.conf', 'logs/*.logs', 'logs/old/*.logs', 'errors.txt' -DestinationPath 'project_backup.zip'"
```
...
```bash
grep -v '^[[:space:]]*$' logs/app.logs > cleaned_app.log
```
